...
import { escalateSiteIssueRequestSchema } from "@/lib/schemas/actions";

export async function POST(request: Request) {
  const parsed = await parseJson(request, escalateSiteIssueRequestSchema);
  if (!parsed.success) {
  ...
  return NextResponse.json(
    await runVoiceAction({
      ...
      overrideIntent: "escalate_site_issue",
      overrideFields: {
